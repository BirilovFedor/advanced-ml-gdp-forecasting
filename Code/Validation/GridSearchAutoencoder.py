...
class GridSearchAutoencoder:
    ...
    def run_grid_search(self, param_grid, top_n = 10):
        ...
        for i, combo in enumerate(combos, 1):
            ...
            total_losses, recon_losses, kl_losses = model.fit(train_data = self.ts.torch_flattened_training_data,
                                    num_epoches = training_params["num_epochs"],
                                    batch_size  = training_params["batch_size"],
                                    optimizer   = self.optimizer(model.parameters(), lr=training_params["learning_rate"]))

            row   = {**parameters, "total_losses": total_losses[-1], "recon_losses": recon_losses[-1], "kl_losses": kl_losses[-1]}
            overall_results.append(row)

        results_df = (pd.DataFrame(overall_results).sort_values("total_losses").reset_index(drop=True))
